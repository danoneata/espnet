...
class ESPnetASRMultimodalModel(AbsESPnetModel):
    def __init__(
        ...
        self.project_emb = torch.nn.Conv1d(1024, 512, kernel_size=1)
    ...
    def forward(
        ...
        assert (
            ...
            == text.shape[0]
            == text_lengths.shape[0]
        ), (speech.shape, speech_lengths.shape, visual.shape, text.shape, text_lengths.shape)
        batch_size = speech.shape[0]
        ...
        # 1. Encoder
        encoder_out, encoder_out_lens = self.asr.encode(speech, speech_lengths)
        _, T, _ = encoder_out.shape
        encoder_visual_out = self.encoder_visual(visual)
        encoder_visual_out = encoder_visual_out.squeeze(-1).squeeze(-1).unsqueeze(1).repeat(1, T, 1)
        # B × T × D
        encoder_out = torch.cat((encoder_out, encoder_visual_out), dim=2)
        encoder_out = encoder_out.permute(0, 2, 1)
        encoder_out = self.project_emb(encoder_out)
        encoder_out = encoder_out.permute(0, 2, 1)
        ...
        if self.asr.rnnt_decoder is not None:
            _ = self.asr._calc_rnnt_loss(encoder_out, encoder_out_lens, text, text_lengths)
